...
kubectl create namespace zt
helm upgrade --install trust-manager jetstack/trust-manager \
    --namespace cert-manager \
    --set crds.keep=false \
    --set app.trust.namespace=zt
```
...
1. Wait for the web identity certificate to be ready:
    ...
    ```bash
    kubectl wait certificate.cert-manager.io zt-controller-web-identity-cert \
        --namespace zt \
...
1. Restart the controller and all routers to pick up the latest certificates:
    ...
    ```bash
    kubectl rollout restart deployment zt-controller -n zt
    kubectl rollout restart deployment zt-router -n zt
    ```
...
```bash
helm upgrade zt-controller hanzozt/zt-controller \
    --install \
    --namespace zt \
    --create-namespace \
    --set clientApi.advertisedHost=ctrl1.zt.example.com \
...
```yaml
clientApi:
    advertisedHost: ctrl1.zt.example.com
    advertisedPort: 32171
    service:
        type: NodePort
```

Visit the Ziti Administration Console (ZAC): https://ctrl1.zt.example.com:32171/zac/

Log in with the `zt` CLI.

<!-- {% raw %} "raw" escapes this code block's handlebars from GH Pages Jekyll, and  escapes the Go template from helm-docs -->
```bash
zt edge login ctrl1.zt.example.com:32171 --yes --username admin --password $(
    kubectl -n zt get secrets zt-controller-admin-secret -o go-template='{{index .data "admin-password" | base64decode }}'
)
```
<!-- {% endraw %} -->
...
Visit the Ziti Administration Console (ZAC): https://ctrl1.zt.example.com/zac/

Log in with the `zt` CLI.

<!-- {% raw %} "raw" escapes this code block's handlebars from GH Pages Jekyll, and  escapes the Go template from helm-docs -->
```bash
zt edge login ctrl1.zt.example.com:443 --yes --username admin --password $(
    kubectl -n zt get secrets zt-controller-admin-secret -o go-template='{{index .data "admin-password" | base64decode }}'
)
```
<!-- {% endraw %} -->
...
```bash
kubectl get secret \
    -n zt zt-controller-admin-secret \
...
This Helm chart's values allow for both operational scenarios: combined and split. The default choice is to expose the combined client and management APIs as the cluster service named `{release}-client`, which is convenient because you can use the `zt` CLI immediately. For additional security, you may shelter the management API by splitting these two sets of features, exposing them as separate API servers. After the split, you can access the management API in several ways:
...
This chart provides a default disabled `zt-controller-prometheus` k8s service for prometheus,
...
| additionalVolumes | list | `[]` | additional volumes to mount to zt-controller container |
...
| cluster.endpoint | string | `""` | required only when joining a cluster: reachable ctrl plane endpoint address of an existing node (example: ctrl1.zt.example.com:443 or zt-ctrl1-controller-ctrl:1280) |
...
| console.altIngress | object | `{}` | override the address printed in Helm release notes if you configured an alternative DNS SAN for the console, i.e. `{"host": "console.zt.example.com", "port": 443}` |
...
| fabric.events.mountDir | string | `"/var/run/zt"` |  |
...
| image.additionalArgs | list | `[]` | additional arguments can be passed directly to the container to modify zt runtime arguments |
| image.args | list | `["{{ include \"configMountDir\" . }}/zt-controller.yaml"]` | args for the entrypoint command |
| image.command | list | `["zt","controller","run"]` | container entrypoint command |
| image.homeDir | string | `"/home/ziggy"` | homeDir for admin login shell must align with container image's ~/.bashrc for zt CLI auto-complete to work |
| image.pullPolicy | string | `"IfNotPresent"` | deployment image pull policy |
| image.repository | string | `"docker.io/hanzozt/zt-controller"` | container image repository for app deployment |
...
```yaml
clientApi:
    advertisedHost: edge.zt.example.com
    ingress:
        enabled: true
    ...
    service:
        ...
        type: ClusterIP
    altDnsNames:
        - "alt-edge.zt.example.com"  # configures SNI routing for ingresses
...
webBindingPki:
    ...
    altServerCerts:
        - mode: certManager
            ...
            altDnsNames:
                - "alt-edge.zt.example.com"  # sets DNS SANs for the cert for SNI routing in controller process
            issuerRef:
                group: cert-manager.io
                kind: ClusterIssuer
                name: cloudflare-dns01-issuer
            mountPath: /etc/zt/alt-server-cert
```
<!-- {% endraw %} -->
...
additionalVolumes:
    - name: my-alt-server-cert
      volumeType: secret
      mountPath: /etc/zt/my-alt-server-cert
      secretName: my-alt-server-cert
...
Assuming your future CM release will be named "cert-manager," your future TM release will be named "trust-manager," and both will be installed in namespace "cert-manager," and your Ziti controller is installed in namespace "zt," you can use these example values to run the provided script to pave the way to installing the version 2 zt-controller chart, which will delete the cert-manager and trust-manager Operators, preserving the CRDs and their associated resources.

```bash
helm pull hanzozt/zt-controller
tar -xvf zt-controller-*.tgz
CM_NAMESPACE=cert-manager \
CM_RELEASE_NAME=cert-manager \
TM_NAMESPACE=cert-manager \
TM_RELEASE_NAME=trust-manager \
ZITI_NAMESPACE=zt \
./zt-controller/files/chown-cert-manager.bash
```
