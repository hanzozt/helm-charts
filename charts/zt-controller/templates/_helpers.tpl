...
Common labels
*/}}
{{- define "zt-controller.labels" -}}
helm.sh/chart: {{ include "zt-controller.chart" . }}
{{ include "zt-controller.selectorLabels" . }}
{{- if .Chart.AppVersion }}
...
Selector labels
*/}}
{{- define "zt-controller.selectorLabels" -}}
app.kubernetes.io/name: {{ include "zt-controller.name" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
app.kubernetes.io/component: "zt-controller"
{{- end }}

{{/*
Create the name of the service account to use
*/}}
{{- define "zt-controller.serviceAccountName" -}}
{{- if .Values.serviceAccount.create }}
{{- default (include "zt-controller.fullname" .) .Values.serviceAccount.name }}
{{- else }}
...
*/}}
{{- define "zt-controller.ctrlPlaneCaDir" -}}
ctrl-plane-cas
{{- end }}
...
*/}}
{{- define "zt-controller.ctrlPlaneCasFile" -}}
ctrl-plane-cas.crt
{{- end }}
...
*/}}
{{- define "zt-controller.ctrlPlaneCasConfigMapName" -}}
{{- if .Values.ctrlPlaneCasBundle.configMapName -}}
{{ .Values.ctrlPlaneCasBundle.configMapName }}
{{- else -}}
{{ include "zt-controller.fullname" . }}-ctrl-plane-cas
{{- end -}}
{{- end }}

{{- define "zt-controller.console" -}}
    {{- if ne (len .Values.console.altIngress) 0 -}}
https://{{ include "zt-controller.tplOrLiteral" (dict "value" .Values.console.altIngress.host "context" .) }}:{{ include "zt-controller.tplOrLiteral" (dict "value" .Values.console.altIngress.port "context" .) }}/zac/
    {{- else if .Values.managementApi.service.enabled -}}
https://{{ include "zt-controller.tplOrLiteral" (dict "value" .Values.managementApi.advertisedHost "context" .) }}:{{ include "zt-controller.tplOrLiteral" (dict "value" .Values.managementApi.advertisedPort "context" .) }}/zac/
    {{- else -}}
...
*/}}
{{- define "zt-controller.getCertManagerAltServerCerts" -}}
{{- $filteredCerts := list -}}
{{- range . -}}
...
*/}}
{{- define "zt-controller.clusterMode" -}}
  {{- $mode := .Values.cluster.mode | trim | lower -}}
  {{- $trustDomain := include "zt-controller.getTrustDomain" . | trim -}}
...
cluster-migrate
  {{- else -}}
    ...
    {{- if eq (len .Values.cluster.endpoint) 0 -}}
      {{- fail "cluster-join requires .Values.cluster.endpoint to be set to a reachable ctrl plane endpoint address of an existing node (example: ctrl1.zt.example.com:443 or zt-ctrl1-controller-ctrl:1280)" -}}
    {{- end -}}
cluster-join
  {{- end -}}
{{- end -}}
...
*/}}
{{- define "zt-controller.lookupVolumeMountPath" -}}
...
*/}}
{{- define "zt-controller.tplOrLiteral" -}}
{{- $value := .value -}}
{{- $context := .context -}}
...
Usage: {{ include "zt-controller.getSpiffeUri" . }}
Returns: spiffe://{trustDomain}/controller/{nodeName}
*/}}
{{- define "zt-controller.getSpiffeUri" -}}
{{- $trustDomain := include "zt-controller.getTrustDomain" . -}}
...
Usage: {{ include "zt-controller.getTrustDomain" . }}
Returns: plain string trust domain (not base64-encoded)
*/}}
{{- define "zt-controller.getTrustDomain" -}}
  {{- $mode := .Values.cluster.mode | trim | lower -}}
  ...
  {{- else if eq $mode "standalone" -}}
    ...
    {{- if ne $legacyTD "" -}}
      {{- $legacyTD -}}
    {{- else -}}
      {{- $secretName := printf "%s-trust-domain" (include "zt-controller.fullname" .) -}}
...
Usage: {{ include "zt-controller.clusterMarkerFile" . }}
*/}}
{{- define "zt-controller.clusterMarkerFile" -}}
