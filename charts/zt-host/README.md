...
```bash
zt edge enroll --jwt /tmp/k8s-tunneler.jwt --out /tmp/k8s-tunneler.json
helm install zt-host hanzozt/zt-host --set-file ztIdentity=/tmp/k8s-tunneler.json
```
...
```bash
helm install zt-host hanzozt/zt-host --set-file ztEnrollToken=/tmp/k8s-tunneler.jwt
```
...
```bash
helm install zt-host hanzozt/zt-host --set secret.existingSecretName=k8s-tunneler-identity
```
...
|-----|------|---------|-------------|
| additionalVolumes | list | `[]` | additional volumes to mount to zt-host container |
...
| image.repository | string | `"hanzozt/zt-host"` |  |
