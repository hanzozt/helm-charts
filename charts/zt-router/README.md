...
zt edge create edge-router "router1" \
...
helm upgrade \
  ...
  hanzozt/zt-router \
    ...
    --set edge.advertisedHost=router1.zt.example.com
```
...
# router-values.yml
ctrl:
  endpoint: zt-controller-ctrl.zt-controller.svc:1280
advertisedHost: router1.zt.example.com
edge:
  advertisedPort: 443
  service:
    type: ClusterIP
...
```bash
zt edge update edge-router "router1" --tunneler-enabled
```
...
edge:
  ...
  additionalListeners:
    - name: router1-edge-wss
      ...
      advertisedHost: router1-wss.zt.example.com  # must be distinct from edge.advertisedHost
      advertisedPort: 443
...
identity:
  altServerCerts:
    - name: alt-server-cert-1
      mode: certManager
      secretName: zt-router1-alt-server-certs1
      additionalListenerName: router1-edge-wss
      mountPath: /etc/zt/alt-server-cert-1
      issuerRef:
...
You don't have to use cert-manager. If you have a TLS secret named `zt-router1-alt-server-certs1` from some other issuer in the same namespace as the router containing the certificate and key, you can use it by setting values like these. You must also configure the additional listener as in the prior example with an advertisedHost that matches a DNS SAN of the alternative certificate.
...
additionalVolumes:
  - name: alt-server-cert-2
    volumeType: secret
    mountPath: /etc/zt/alt-server-cert-2
    secretName: zt-router1-alt-server-cert-2
...
additionalVolumes:
  - name: alt-server-cert-3
    ...
    mountPath: /etc/zt/alt-server-cert-3

identity:
  altServerCerts:
    - mode: localFile
      serverCert: /etc/zt/alt-server-cert-3/server3.crt
      serverKey: /etc/zt/alt-server-cert-3/server3.key
```
...
|-----|------|---------|-------------|
| additionalVolumes | list | `[]` | additional volumes to mount to zt-router container |
...
| configFile | string | `"zt-router.yaml"` | filename of router config YAML |
| configMountDir | string | `"/etc/zt/config"` | writable mountpoint where read-only config file is projected to allow router to write ./endpoints.yml statefile in same dir |
...
| ctrl.endpoint | string | `""` | optionally override the endpoint(s) from the enrollment token, e.g., ctrl.zt.example.com:443 |
...
| identityMountDir | string | `"/etc/zt/identity"` | read-only mountpoint for router identity secret specified in deployment for use by router run container |
...
| image.repository | string | `"docker.io/hanzozt/zt-router"` | container image tag for deployment |
...
| linkListeners.transport.containerPort | string | `"{{ .Values.edge.containerPort }}"` | cluster service target port on the container; default is to share a listener with the edge, providing zt-link ALPN |
...
| livenessProbe | object | `{"active":true,"command":["/bin/sh","-c","zt agent stats"],"failureThreshold":5,"initialDelaySeconds":10,"periodSeconds":10,"successThreshold":1,"timeoutSeconds":5}` | deployment container liveness probe config |
| livenessProbe.active | bool | `true` | enables the liveness probe for the deployment |
| livenessProbe.command | list | `["/bin/sh","-c","zt agent stats"]` | command to run to determine if the container is ready |
...
| readinessProbe | object | `{"active":true,"command":["/bin/sh","-c","zt agent stats"],"failureThreshold":5,"initialDelaySeconds":10,"periodSeconds":10,"successThreshold":1,"timeoutSeconds":5}` | deployment container readiness probe config |
| readinessProbe.active | bool | `true` | enables the readiness probe for the deployment |
| readinessProbe.command | list | `["/bin/sh","-c","zt agent stats"]` | command to run to determine if the container is ready |
