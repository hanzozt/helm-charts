...
* <https://github.com/hanzozt/zt-tunnel-sdk-c>
...
```console
helm install zt-edge-tunnel hanzozt/zt-edge-tunnel --set-file ztEnrollToken=/tmp/k8s-tunneler.jwt
```
...
```console
zt-edge-tunnel enroll --jwt /tmp/k8s-tunneler.jwt --identity /tmp/k8s-tunneler.json
helm install zt-edge-tunnel hanzozt/zt-edge-tunnel --set-file ztIdentity=/tmp/k8s-tunneler.json
```
...
```console
kubectl create secret generic k8s-tunneler-identity --from-file=ztIdentity=k8s-tunneler.json
```

Deploy with the existing secret:

```console
helm install zt-edge-tunnel hanzozt/zt-edge-tunnel --set secret.existingSecretName=k8s-tunneler-identity
```
...
```console
helm install zt-edge-tunnel hanzozt/zt-edge-tunnel \
...
helm install zt-edge-tunnel hanzozt/zt-edge-tunnel \
  --set-file ztIdentity=/tmp/k8s-tunneler.json \
...
Helm chart, which can pull the new `zt-edge-tunnel` image Version needed in beforehand.
...
|-----|------|---------|-------------|
| additionalVolumes | list | `[]` | additional volumes to mount to zt-edge-tunnel container |
| affinity | object | `{}` |  |
...
| image.repository | string | `"hanzozt/zt-edge-tunnel"` |  |
| image.tag | string | `""` |  |
| imagePullSecrets | list | `[]` |  |
| livenessProbe.exec.command[0] | string | `"/bin/bash"` |  |
| livenessProbe.exec.command[1] | string | `"-c"` |  |
| livenessProbe.exec.command[2] | string | `"if (zt-edge-tunnel tunnel_status | jq '.Success'); then true; else false; fi"` |  |
...
| log.ztLevel | int | `3` | Ziti log level, from 0 to 6 (see README.md Reference) |
| nameOverride | string | `""` |  |
| nodeSelector | object | `{}` | constrain worker nodes where the zt-edge-tunnel pod can be scheduled |
...
| tolerations | list | `[]` |  |
| ztEnrollToken | string | `""` | JWT to enroll a new identity and write in the PVC |
| ztIdentity | string | `""` | JSON of an enrolled identity to write in the PVC |
