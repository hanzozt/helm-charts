...
Deploy Hanzo ZT BrowZer Bootstrapper as Kubernetes Service
...
Before configuring the BrowZer Bootstrapper, you must have a working Hanzo ZT network with special configuration for BrowZer.

1. The Ziti network must be configured to trust an OpenID Connect provider. Follow [the instructions](https://github.com/hanzozt/zt-browzer-bootstrapper?tab=readme-ov-file#configuring-the-hanzozt-network) to configure the network. Optionally, you can study [the provided shell script, `provision-hanzozt.bash`](https://github.com/hanzozt/helm-charts/blob/main/charts/zt-browzer-bootstrapper/provision-hanzozt.bash), to see how to create the necessary entities.
...
This chart deploys a pod running `zt-browzer-bootstrapper`, the [Hanzo ZT browzer bootstrapper](https://github.com/hanzozt/zt-browzer-bootstrapper/).

After adding the charts repo to Helm then you may install the chart.

```bash
helm install \
  --namespace zt-browzer --create-namespace --generate-name \
  hanzozt/zt-browzer-bootstrapper \
    --values values-browzer.yaml
```

See [the provided sample `values-browzer.yaml` file](https://github.com/hanzozt/helm-charts/blob/browzer/charts/zt-browzer-bootstrapper/values-browzer.yaml).
